...
vis:command_register('ff', function(argv, force, win, cursor, range)
	local currentFile = win.file.path
	local handle = io.popen('fzf')
	local result = handle:read("*a")
	handle:close()
	vis:command('e ' .. (result or ''))

	-- Really hacky redraw, but it werks
	if result == '' then
		vis:command('e! temp')
		vis:command('e! ' .. (currentFile or ''))
	else
		vis:command('e! temp')
		vis:command('e! ' .. (result or ''))
	end
end, 'Run FZF and open file')

vis:operator_new('gs', function(file, range, pos)
	local text = file:content(range)
	local command = assert(io.popen(text))
	local output = command:read('*a')
	command:close()
	file:delete(range)
	file:insert(range.start, output)
	return range.start
end, 'Send selection to shell and print output')
